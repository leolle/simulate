...
def fillHolding(d, nextd, tradeDates, df_holdings, df_holdingCash,
                df_totalReturnFactor):
    """ update holding shares between two rebalance date
    new holding = holding[d:nextd] * adjust factor[d:nextd] / adjust factor[d]
    Parameters
    ----------
    d: date
    ...
    nextd: date
        next rebalance date
    tradeDates: pd.date_range
        rebalance date index
    df_holdings: pd.DataFrame
        holding positions(shares)
    df_holdingCash: pd.DataFrame
        holding cash value
    df_totalReturnFactor: pd.DataFrame
        adjustment factors

    Returns
    -------
    holdingCash: updated holding cash
    holding: updated holdings
    """
    if nextd > d:
        holdingDates = tradeDates[(tradeDates >= d) & (tradeDates <= nextd)]
        df_holdings.ix[holdingDates] = np.tile(df_holdings.ix[d],
                                               (len(holdingDates), 1))
        df_holdingCash.ix[holdingDates] = np.tile(df_holdingCash.ix[d],
                                                  (len(holdingDates), 1))
...
def SIMPLE_SIMULATE_DAILY_TRADE_CHN_STK(
        beginDate, endDate, initialHolding, df_targetPortfolioWgt,
    ...
    if isinstance(initialHolding, gftIO.GftTable):
        ...
        beginDate = gsUtils.alignDate(
            df_initialHolding.index[0], allDates, method='ffill')
    ...
    if len(sigDates) > 0:
        ...
        if execDelayPeriods > 0:
            idxs = np.array(
                gsUtils.alignDate(
                    sigDates, allDates, method='bfill', returnidx=True),
                dtype=float)
    ...
    df_markToMarketPrice = df_markToMarketPrice.reindex(
        priceDates, allSymbols, fill_value=NA)
    df_totalReturnFactor = df_totalReturnFactor.reindex(
        priceDates, allSymbols, fill_value=1.).fillna(1.)
    df_executePrice = df_executePrice.reindex(
        priceDates, allSymbols, fill_value=NA)
    df_execPriceReturn = df_execPriceReturn.reindex(
        priceDates, allSymbols, fill_value=NA)
    df_tradeVolume = df_tradeVolume.reindex(
        priceDates, allSymbols, fill_value=0.)

    if initHldIsCash:
        df_initialHolding = pd.DataFrame(
            initialHolding, index=[beginDate], columns=[cashSymbol])
    df_initialHolding = df_initialHolding.reindex(columns=np.append(
        allSymbols, cashSymbol)).fillna(0.)
        ...
        for i in range(len(rebDates)):
            ...
            if (s_sellOrderWgt < 0).any():
                s_sellOrder = gsUtils.roundToLot(
                    s_sellOrderWgt / s_currentHoldingWgt.where(
                        s_currentHoldingWgt > 0, 1.0) * df_holdings.ix[d],
                    lotSize)
            ...
            if (s_buyOrderWgt > 0).any():
                ...
                if canBuyWgt > 0:
                    s_buyOrder = gsUtils.roundToLot(
                        (min(canBuyWgt / s_buyOrderWgt.sum(),
                             1.0) * s_buyOrderWgt * totalValue /
                         (1 + buyCommission) / df_executePrice.ix[d]).fillna(0),
                        lotSize)
    ...
    df_portfolioValue.ix[:, 0] = (
        df_holdings * df_markToMarketPrice.ix[tradeDates]).sum(
...
dataPack = gftIO.zload(
    r'd:\Wuwei\Project\simulator\data\monthly_rebalance_data_execute_at_next_open_1.pkl'
)
