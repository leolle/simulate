...
import os
#os.chdir("D:/Dev/WaveFront/PythonScripts")    
dataPack = gftIO.zload(r'd:\Wuwei\Project\simulator\data\monthly_rebalance_data_same_as_R_5.pkl')
#beginDate = dataPack['x0']
#endDate = dataPack['x1']
#initialHolding = dataPack['x2']
#df_targetPortfolioWgt = dataPack['x3']
#df_markToMarketPrice = dataPack['x4']
#df_totalReturnFactor = dataPack['x5']
#df_executePrice = dataPack['x6']
#df_execPriceReturn = dataPack['x7']
#df_tradeVolume = dataPack['x8']
#dict_tradingParam = dataPack['x9']
#
#dt_begin = dataPack['x0']
#dt_end = dataPack['x1']
#initial_holding_position = dataPack['x2']
#df_w_target_portfolio_weight = dataPack['x3']
#df_w_market_price = dataPack['x4']
#df_w_total_return_factor = dataPack['x5']
#df_w_execute_price = dataPack['x6']
#df_w_execute_price_return = dataPack['x7']
#df_w_trade_volume = dataPack['x8']
#dict_trading_param = dataPack['x9']
...
def simpleSimulate(beginDate, endDate, initialHolding, df_targetPortfolioWgt, 
    ...
    if len(allDates[(allDates>=beginDate) & (allDates<=endDate)]) < 1:
        raise TypeError('no trading date falls between begindate and enddate')
    endDate = allDates[allDates<=endDate][-1]
    if beginDate > endDate:
        raise TypeError('beginDate should be less than endDate')
    
    initHldIsCash = True
    if isinstance(initialHolding, gftIO.GftTable):
        df_initialHolding = initialHolding.asMatrix()
        if df_initialHolding.shape[0] < 1:
            raise TypeError('no init holding is provided')
        initHldIsCash = False
        ...
        if pd.isnull(beginDate):
            raise TypeError('do not have close price for the date of initHld')
        df_initialHolding.index = [beginDate]
    else:
        beginDate = gsUtils.alignDate(beginDate, allDates, method='bfill')
        if pd.isnull(beginDate):
            raise TypeError('beginDate should be less than the last trading date')
            
    if (df_targetPortfolioWgt<0).any(axis=1).any():
        raise TypeError('Do not support stock short selling and cash borrowing')
    ...
    if len(sigDates) > 0:
        ...
        if execDelayPeriods > 0:
            ...
            if sum(idxs_nonnan_flag) < 1:
                raise TypeError("no trade date after the execute delay shift")   
        ...
        if len(rebDates) > 0:
            if initHldIsCash:
                ...
                if pd.isnull(beginDate):
                    raise TypeError('beginDate is null after shift')
    
    tradeDates = allDates[(allDates>=beginDate) & (allDates<=endDate)]
    beginDate = tradeDates[0]
    endDate = tradeDates[-1]
    if beginDate > endDate:
        raise TypeError("Begin date is larger than end date after the date processing!")
    ...
    if len(np.setdiff1d(holdingSymbols, allSymbols)) > 0:
        raise TypeError("Initial Portfolio has non A-share stocks!")
    if len(np.setdiff1d(portfolioSymbols, allSymbols)) > 0:
        raise TypeError("Target Portfolio has non A-share stocks!")
    ...
    result = {}
    result[gsConst.Const.Holding] = pd.concat([df_holdings.where(~np.isclose(df_holdings, 0), NA), df_holdingCash], axis=1)
    result[gsConst.Const.PortfolioValue] = df_portfolioValue
    result[gsConst.Const.Weights] = df_weights.where(~np.isclose(df_weights, 0), NA)
...
result = simpleSimulate(dataPack['begin_date'], \
                                    dataPack['end_date'], \
